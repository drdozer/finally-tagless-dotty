...
object Value {
  ...
  implied ValueAndThenPosition[Buffer, A] for ParseOneThenOther[Value[Buffer, A], ParseAt[Buffer], Value[Buffer, A]] =
  ...
  implied PositionAndThenValue[Buffer, B] for ParseOneThenOther[ParseAt[Buffer], Value[Buffer, B], Value[Buffer, B]] =
    (lhs, rhs) => (buff, pos) => lhs(buff, pos)(new {
