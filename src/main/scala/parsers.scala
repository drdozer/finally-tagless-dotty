...
trait Parser[R] {
  ...
  /** Combine two parsers so that the first one will attempt to match, and if it does, the second one will.
    ...
    */
  def (lhs: R) andThen (rhs: R): R            // semigroup, non-symmetrical as consumes left-to-right
  ...
  /** Combine two parsers so that the first one is tried, and if it fails, the second one is tried.
    ...
    */
  def (lhs: R) orAlternatively (rhs: R): R    // semigroup, non-symmetrical as first alternative takes prescedence if rhs intersects lhs
